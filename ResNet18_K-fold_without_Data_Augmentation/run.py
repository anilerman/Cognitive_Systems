...
import train
import test  # Ensure this import is correct
...
def main():
    ...
    if k_fold:
        ...
        for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
            ...
            test.test_model(trained_model, val_loader, device)  # Correctly call test_model from test module
            plot_metrics(metrics)
            plot_differences(trained_model, val_loader, device, config["data"]["classes"])
            model_save_path = os.path.join(config['training']['checkpoint_dir'], f'best_model_fold_{fold}.pth')
            torch.save(trained_model.state_dict(), model_save_path)
    else:
        model_instance = model_module.get_model(config["model"], num_classes).to(device)
        train_loader, val_loader, test_loader = create_data_loaders(config['data'])
        trained_model, metrics = train.train_model(model_instance, train_loader, val_loader, config)
        test.test_model(trained_model, test_loader, device)  # Correctly call test_model from test module
        plot_metrics(metrics)
        plot_differences(trained_model, test_loader, device, config["data"]["classes"])
        model_save_path = os.path.join(config['training']['checkpoint_dir'], 'best_kfold_without_Aug_.pth')
        torch.save(trained_model.state_dict(), model_save_path)
